...
class ContrailDataCenterInterconnect(contrail.ContrailResource):
    PROPERTIES = (
        NAME, FQ_NAME, DATA_CENTER_INTERCONNECT_BGP_ADDRESS_FAMILIES, DATA_CENTER_INTERCONNECT_BGP_ADDRESS_FAMILIES_FAMILY, DISPLAY_NAME, DATA_CENTER_INTERCONNECT_BGP_HOLD_TIME, PERMS2, PERMS2_OWNER, PERMS2_OWNER_ACCESS, PERMS2_GLOBAL_ACCESS, PERMS2_SHARE, PERMS2_SHARE_TENANT, PERMS2_SHARE_TENANT_ACCESS, DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER, DATA_CENTER_INTERCONNECT_CONFIGURED_ROUTE_TARGET_LIST, DATA_CENTER_INTERCONNECT_CONFIGURED_ROUTE_TARGET_LIST_ROUTE_TARGET, ANNOTATIONS, ANNOTATIONS_KEY_VALUE_PAIR, ANNOTATIONS_KEY_VALUE_PAIR_KEY, ANNOTATIONS_KEY_VALUE_PAIR_VALUE, TAG_REFS, LOGICAL_ROUTER_REFS, VIRTUAL_NETWORK_REFS, GLOBAL_SYSTEM_CONFIG
    ) = (
        'name', 'fq_name', 'data_center_interconnect_bgp_address_families', 'data_center_interconnect_bgp_address_families_family', 'display_name', 'data_center_interconnect_bgp_hold_time', 'perms2', 'perms2_owner', 'perms2_owner_access', 'perms2_global_access', 'perms2_share', 'perms2_share_tenant', 'perms2_share_tenant_access', 'data_center_interconnect_vxlan_network_identifier', 'data_center_interconnect_configured_route_target_list', 'data_center_interconnect_configured_route_target_list_route_target', 'annotations', 'annotations_key_value_pair', 'annotations_key_value_pair_key', 'annotations_key_value_pair_value', 'tag_refs', 'logical_router_refs', 'virtual_network_refs', 'global_system_config'
    )
    ...
    properties_schema = {
        ...
        DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER: properties.Schema(
            properties.Schema.STRING,
            _('DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER: attributes.Schema(
            _('DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER.'),
        ),
    ...
    def handle_create(self):
        ...
        if self.properties.get(self.DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER) is not None:
            obj_0.set_data_center_interconnect_vxlan_network_identifier(self.properties.get(self.DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER))
        ...
        if prop_diff.get(self.PERMS2) is not None:
            ...
            if prop_diff.get(self.PERMS2, {}).get(self.PERMS2_SHARE) is not None:
                for index_1 in range(len(prop_diff.get(self.PERMS2, {}).get(self.PERMS2_SHARE))):
                    ...
                    obj_1.add_share(obj_2)
            obj_0.set_perms2(obj_1)
        if prop_diff.get(self.DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER) is not None:
            obj_0.set_data_center_interconnect_vxlan_network_identifier(prop_diff.get(self.DATA_CENTER_INTERCONNECT_VXLAN_NETWORK_IDENTIFIER))
